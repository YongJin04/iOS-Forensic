...
# 백엔드 모듈(KakaoTalk 분석 기능)을 임포트합니다.
from artifact_analyzer.messenger.kakaotalk.kakaotalk_analyzer import KakaoTalkAnalyzer


def display_kakaotalk(parent_frame, backup_path):
    """카카오톡 메시지 데이터를 표시하는 함수입니다.
    
    인자:
      parent_frame: KakaoTalk 데이터를 출력할 부모 위젯(프레임)
      backup_path: iOS 백업 파일의 경로 (KakaoTalk .sqlite 데이터베이스 찾는 용도)
    """
    ...
    # 최상단에 '카카오톡 메시지 분석' 헤더를 추가합니다.
    ttk.Label(parent_frame, text="카카오톡 메시지 분석", style="HeaderLarge.TLabel").pack(anchor="w", pady=(0, 10))
    
    # 백엔드의 KakaoTalkAnalyser 클래스를 이용해 KakaoTalk 분석기를 초기화합니다.
    try:
        kakaotalk_analyzer = KakaoTalkAnalyzer(backup_path)
        # 카카오톡 데이터베이스에 연결 시도. 실패할 경우 에러 메시지 출력 후 함수 종료.
        if not kakaotalk_analyzer.connect_to_message_db():
            messagebox.showerror("오류", "KakaoTalk Message.sqlite 데이터베이스를 찾을 수 없거나 연결할 수 없습니다.")
            ttk.Label(parent_frame, text="KakaoTalk Message.sqlite 데이터베이스를 찾을 수 없거나 연결할 수 없습니다.", 
                      style="ErrorText.TLabel").pack(pady=20)
            return
        if not kakaotalk_analyzer.connect_to_talk_db():
            messagebox.showerror("오류", "KakaoTalk Talk.sqlite 데이터베이스를 찾을 수 없거나 연결할 수 없습니다.")
            ttk.Label(parent_frame, text="KakaoTalk Talk.sqlite 데이터베이스를 찾을 수 없거나 연결할 수 없습니다.", 
                      style="ErrorText.TLabel").pack(pady=20)
            return
    except Exception as e:
        # 초기화 과정에서 예외 발생 시 에러 메시지 출력 후 함수 종료.
        messagebox.showerror("오류", f"KakaoTalk Analyzer Initialize 중 오류가 발생했습니다: {str(e)}")
    ...
    # 현재 선택된 대화 상대의 정보를 저장할 변수 (chatId와 handle_id)
    selected_conversation = {"chatId": None, "handle_id": None}
    ...
    def show_conversation(event=None):
        ...
        selected_conversation["chatId"] = conversation_data["chatId"]
        ...
        # 선택된 채팅방의 메시지 데이터를 가져옵니다.
        messages = kakaotalk_analyzer.get_conversation_messages(conversation_data["chatId"])
        if not messages:
    ...
    def create_message_bubble(parent, message, canvas):
        ...
        # 메시지 날짜 정보를 하단에 출력
        ttk.Label(content_frame, text=message["sentAt_string"], style="SmallText.TLabel").pack(pady=(5, 0))
    ...
    try:
        ...
        if not conversations:
            ...
            # 전체 KakaoTalk 메시지를 DataFrame으로 불러와 통계 정보를 계산 후 표시
            all_messages_df = kakaotalk_analyzer.get_all_kakaotalk_messages()
            stats = kakaotalk_analyzer.get_kakaotalk_stats(all_messages_df)
            
            # 통계 정보를 표시할 프레임 생성 및 내용 추가
            stats_frame = ttk.Frame(parent_frame, style="Card.TFrame", padding=10)
            stats_frame.pack(fill="x", pady=10)
            ttk.Label(stats_frame, 
                      text=f"통계: 총 {stats['total']}개 메시지 (발신: {stats['sent']}개, 수신: {stats['received']}개, 채팅방: {stats['contacts']}개)", 
    ...
    # --------------------------
    # '전체 카카오톡 목록 보기' 버튼 및 관련 기능
    # --------------------------
    
    # 전체 KakaoTalk 메시지를 테이블 형식으로 출력하는 함수입니다.
    def show_all_kakaotalk_rooms():
        # 우측 메시지 프레임 초기화
        for widget in message_frame.winfo_children():
            widget.destroy()
        
        # 제목 라벨 추가
        ttk.Label(message_frame, text="All Messages", style="Header.TLabel").pack(anchor="w", pady=(0, 10))
        
        # 테이블을 담을 프레임 생성
        table_frame = ttk.Frame(message_frame)
        table_frame.pack(fill="both", expand=True)
        
        # Treeview를 사용해 테이블 형태의 Message 목록을 구성
        columns = ("번호", "날짜", "방향", "연락처", "내용", "첨부파일")
        ...
        tree.heading("첨부파일", text="첨부파일")
        tree.column("번호", width=50, anchor="center")
        tree.column("날짜", width=100, anchor="center")
        tree.column("방향", width=50, anchor="center")
        tree.column("연락처", width=100, anchor="w")
        tree.column("내용", width=300, anchor="w")
        tree.column("첨부파일", width=100, anchor="w")
        
        # 전체 KakaoTalk Messages를 DataFrame으로 가져와서 Treeview에 추가
        all_messages_df = kakaotalk_analyzer.get_all_kakaotalk_messages()
        for _, row in all_messages_df.iterrows():
            tree.insert("", "end", values=(
                ...
                row['attachment'] if pd.notna(row['attachment']) else "",
                # row['readAt'],
                # row['prevId']
            ))
    ...
    # 좌측 프레임에 '전체 카카오톡 목록 보기' 버튼을 추가하여, 버튼 클릭 시 전체 목록을 표시하도록 설정
    all_message_button = ttk.Button(contact_frame, text="전체 카카오톡 목록 보기", command=show_all_kakaotalk_rooms)
    all_message_button.pack(fill="x", pady=10)
